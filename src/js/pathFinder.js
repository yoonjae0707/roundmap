...
export class SubwayPathFinder {
  ...
  runDijkstra(startNode, endStationId, endLines, mode, isAccessibleOnly) {
    ...
    while (queue.length > 0) {
      ...
      for (const neighbor of neighbors) {
        ...
        // 해당 목적지역 자체가 교통약자 미지원(엘리베이터 없음)이면 통과하지 않음
        const [neighborStationId, neighborLineId] = neighbor.to.split('_');
        if (isAccessibleOnly && neighborStationId !== endStationId) {
          const destStation = stations[neighborStationId];
          const accessibleData = destStation?.accessible[neighborLineId] || destStation?.accessible[destStation.lines[0]];
          if (destStation && accessibleData && !accessibleData.hasElevator) {
            continue;
          }
        }
  ...
  reconstructPath(endNode, startNode, previous, totalDist) {
    ...
    for (let i = 0; i < rawPath.length; i++) {
      ...
      // 호선별 접근성 데이터를 고려하여 교통약자 접근성 이슈 체크
      const accessibleData = station.accessible[line] || station.accessible[station.lines[0]];
      if (accessibleData && !accessibleData.hasElevator) {
        accessibilityIssues.push(`${station.name} (${line}호선 엘리베이터 없음)`);
      }
      ...
      if (i > 0) {
        ...
        if (prevStationId === stationId) {
          ...
          if (transfers[stationId]?.accessible === false) {
            accessibilityIssues.push(`${station.name} 내 ${prevLine} ↔ ${line} 환승 동선 휠체어 리프트 이용 필요`);
          }
        } else {
          // 단순 이동
